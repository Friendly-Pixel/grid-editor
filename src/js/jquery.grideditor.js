    ...
    self.each(function(baseIndex, baseElem) {
        ...
        var settings = $.extend({
            'new_row_layouts'   : [
                ...
                // Extended column layouts for all avaiable media types
                // specified in colClasses
        ...
        function setup() {
            ...
            // Create visible layouts
            visibleLayouts = $('<div class="btn-group" />');
            $.each(settings.new_row_layouts, function(j, layouts) {
                if (typeof(layouts[0]) == 'object') {
                    layout = new Array();
                    for (var i=0, len=layouts.length; i<len; i++) {
                        layout[i] = layouts[i][0];
                    }
                } else {
                    layout = layouts;
                }
                var get_layout = layout;
                var btn = $('<a class="btn btn-sm" data-layout="'+j+'" />')
                    .attr('title', 'Add row ' + layout.join('-'))
                    .appendTo(visibleLayouts);
        ...
        function createRowControls() {
            canvas.find('> .row, .column > .row').each(function() {
                var row = $(this);
                ...
                createTool(drawer, 'Settings', '', 'fa fa-cog', function() {
                    $(this).parent().find('.ge-details').toggle();
                });
                ...
                createTool(drawer, 'Duplicate row', '', 'fa fa-clone', function() {
                    // however, don't clone rows, it runs into problems with sortables
                    var parent_row = getParentRow(this);
                    var row = createRow().insertAfter(parent_row);
                    parent_row.find('> .column').each(function(){
                        $(this).clone(true,true).appendTo(row);
                    });
                    init();
                    if (row[0].scrollIntoView) row[0].scrollIntoView({behavior: 'smooth'});
                });

                createTool(drawer, 'Add row', 'ge-add-row', 'fa fa-plus-square', function() {
                    ContextDrawer(row.find('> .ge-tools-drawer'),visibleLayouts);
                });

                createTool(drawer, 'Add column', 'ge-add-column', 'fa fa-columns', function() {
                    var row = getParentRow(this);
        ...
        function createColControls() {
            canvas.find('.row > .column').not('.row.ge-row-icon > .column').each(function() {
                var col = $(this);
                ...
                createTool(drawer, 'Duplicate col', '', 'fa fa-clone', function() {
                    var col = getParentColumn(this);
                    col.clone(true).appendTo(col.parent());
                });

                createTool(drawer, 'Add row', 'ge-add-row', 'fa fa-plus-square', function() {
                    var column = getParentColumn(this);
                    ContextDrawer(column.find('> .ge-tools-drawer'),visibleLayouts);
                });
        ...
        function getParentColumn(object) {
            return $(object).closest('.column');
        }

        function getParentRow(object) {
            return $(object).closest('.row');
        }

        function getParentDetails(object) {
            var column = getParentColumn(object);
            return column.find('.ge-details');
        }
        ...
        function ContextDrawer(container,items) {
            var menu = container.find('.ge-menu');
            if (menu.find('.btn-group').length) {
                menu.remove();
                return;
            }
            var menu = $('<div class="ge-menu" />');
            items.appendTo(menu);
            menu.appendTo(container);
            items.find('.btn').off().on('click',function(){
                var layout = $(this).data('layout');
                var row = createRow();
                if (container.parent().hasClass('row')) {
                    row.insertAfter(container.parent());
                } else {
                    row.appendTo(container.parent());
                }
                createColumnLayout(row,layout);
            });
            return menu;
        }
        ...
        function createColumnLayout(row,layout) {
            settings.new_row_layouts[layout].forEach(function(value,index) {
                var column = $('<div/>');
                var classname = '';
                if (typeof(settings.new_row_layouts[layout][index]) == 'number') {
                    classname += colClasses[index]+settings.new_row_layouts[layout][index]+' ';
                } else {
                    settings.new_row_layouts[layout][index].forEach(function(size,key) {
                        classname += colClasses[key]+size+' ';
                    });
                }
                column.addClass(classname.trim()).append(createDefaultContentWrapper().html(
                    getRTE(settings.content_types[0]).initialContent)
                );
                column.appendTo(row);
            });
